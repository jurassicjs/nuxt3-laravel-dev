...
async function loadNuxt(opts) {
  ...
  const nearestNuxtPkg = await Promise.all(["nuxt3", "nuxt-edge", "nuxt"].map((pkg2) => resolvePackageJSON(pkg2, { url: opts.cwd }).catch(() => null))).then((r) => r.filter(Boolean).sort((a, b) => b.length - a.length)[0]);
  if (!nearestNuxtPkg) {
